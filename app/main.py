...
class ASRService:
    ...
    def infer_chunk(self, pcm_int16: bytes, session: StreamSession, is_final: bool = False) -> str:
        if pcm_int16:
    ...
    try:
        while True:
            ...
            if "bytes" in message and message["bytes"] is not None:
                ...
                text = await loop.run_in_executor(None, asr_service.infer_chunk, message["bytes"], session, False)
                ...
                if event.get("event") == "end":
                    await loop.run_in_executor(None, asr_service.infer_chunk, b"", session, True)
